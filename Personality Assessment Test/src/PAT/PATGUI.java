...
public class PATGUI extends JFrame implements ActionListener {
	...
	public PATGUI(){
		...
		mainJP.setLayout(new FlowLayout(CENTER, 0, 50));
		...
		mainJP.add(jlSession);		
		//mainJP.add(jlAnswer);		
		mainJP.add(jTextArea);		
		mainJP.add(jbNextQuestion);		
		mainJP.add(jbDisplayQA);
		mainJP.add(jbDisplayLongestWord);
		mainJP.add(jbDisplayLWordAB);
	...
	switch(btnFace){
		...
		case "Next Question":
			tfHandler.appendToFile("PAT.myFiles/personality.txt", jTextArea.getText());
			question.getSessionNumber();
			question.Question();
			jbCreateFile.setVisible(false);
			jTextArea.setVisible(true);
			jlSession.setVisible(true);
			jlSpace.setVisible(true);
			jlAnswer.setVisible(true);
			jbNextQuestion.setVisible(true);
			break;
	}
